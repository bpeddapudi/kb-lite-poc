import { useCallback, useMemo, useEffect, useId, useRef, useState } from 'react'
import { Link } from 'react-router-dom'
import './MapboxPreloaderExperiment.css';
...
const locations = [
  { name: 'Singapore', region: 'apac', x: 72, y: 57, delay: 0.2 },
  { name: 'Tokyo', region: 'apac', x: 82, y: 40, delay: 0.6 },
  { name: 'Sydney', region: 'apac', x: 84, y: 78, delay: 1.0 },

  { name: 'London', region: 'emea', x: 44, y: 30, delay: 0.3 },
  { name: 'Frankfurt', region: 'emea', x: 49, y: 34, delay: 0.7 },

  { name: 'Ashburn', region: 'amer', x: 24, y: 39, delay: 0.5 },
  { name: 'Silicon Valley', region: 'amer', x: 12, y: 43, delay: 0.9 }
];
...
function EquinixEmblemPathLoader({ className = '', theme = 'dark' }) {
  ...
  return (
    <div className={`mx-auto w-[min(320px,92vw)] ${className}`} aria-hidden>
      <div className="page">
        {/* Map placeholder */}
        <div className={`map ${loaded ? 'show' : ''}`}>
          <h2>Mapbox goes here</h2>
        </div>

        {/* Loader */}
        <div className={`loader ${loaded ? 'hide' : ''}`}>
          <div ref={wrapRef} className="world">
...
function LoaderPreviewCard({ selected, onClick, title, description, children }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-xl border p-4 text-left transition focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/50 ${selected
...
function IrisMapFrame({ phase, theme, children }) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl border shadow-2xl ${frameBorder}`}>
      ...
      <div
        className={`absolute inset-0 z-10 ${phase === 'ready' ? 'pointer-events-none opacity-0' : 'opacity-100'
      ...
      {phase === 'ready' ? (
        <div className={`absolute inset-0 z-0 flex min-h-0 flex-col ${isLight ? 'bg-slate-100' : 'bg-[#0c1220]'}`}>
          ...
          <div
            className={`flex shrink-0 items-center justify-between border-t px-3 py-2 text-[10px] ${isLight ? 'border-slate-200/90 bg-white/95 text-slate-500' : 'border-white/10 bg-black/50 text-white/50'
              }`}
          >
...
export function MapboxPreloaderExperiment() {
      ...
      <header className="border-b border-white/10 px-4 py-3 sm:px-6">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-sm font-semibold text-white/90">Mapbox preloader — direction mocks</h1>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <button
                ...
                className={`rounded-md px-3 py-1 text-xs font-medium transition ${mapTheme === 'dark' ? 'bg-white/15 text-white shadow-sm' : 'text-white/45 hover:text-white/75'
                  }`}
              >
                Dark
              </button>
              <button
                ...
                className={`rounded-md px-3 py-1 text-xs font-medium transition ${mapTheme === 'light' ? 'bg-white/15 text-white shadow-sm' : 'text-white/45 hover:text-white/75'
                  }`}
              >
                Light
              </button>
            ...
            <div className={`flex w-full max-w-md flex-col items-center ${stackGap}`}>
              {showEquinixEmblem ? (
                <img
                  ...
                  alt=""
                  className={`h-10 w-auto sm:h-12 ${mapTheme === 'light'
